...
-define(CLIENTTIMEOUT,
	1000).        % timeout period in ms for client calls

-define(SIMLOAD,
	500).               % to simulate server load, add 0..SIMLOAD ms delay before responding

-define(INDENT,
	"             ").    % how many spaces to indent per column

-export([allocate/0, clear/0, deallocate/1, show/0,
	 start/0, stop/0]).
...
init() ->
    ...
    io:format("Server up at ~w with ~p~n",
	      [self(), Frequencies]),
    loop(Frequencies).

get_frequencies() ->
    [10, 11, 12, 13, 14].       % hardcoded, for now
...
loop(Frequencies) ->                            % The Main Loop
    receive
	  ...
	  Pid !
	    {reply,
	     Reply},     % Reply can be {error, already_allocated}, {ok, Freq}, {error, no_frequency}
	  loop(NewFrequencies);
      {request, Pid, {deallocate, Freq}} ->
	  {NewFrequencies, Reply} = deallocate(Frequencies,
					       {Freq, Pid}),
	  simulate_load(),
	  Pid !
	    {reply,
	     Reply},   % Reply can be {error, not_allocated}, ok
	  loop(NewFrequencies);
      {request, Pid,
       show} ->     % show current frequency allocations
	  Pid ! {reply, Frequencies}, loop(Frequencies);
      {'EXIT', Pid,
       _Reason} ->     %% a client linked to the server has exited!
	  NewFrequencies = exited(Frequencies,
				  Pid), % {Free,Allocated}
...
allocate({[], Allocated}, _Pid) ->
    {{[], Allocated},
     {error,
      no_frequency}};               % no free freqs, return error msg
allocate({[Freq | Free], Allocated} = Frequencies,
	 Pid) ->
    link(Pid),                                              % while a client has a frequency, get notified if it dies.
    case lists:keymember(Pid, 2,
			 Allocated)               % see if this Pid already has a frecuency (key in 2nd pos)
	of
      true ->
	  {Frequencies,
	   {error,
	    already_allocated}};        % will not allocate second freq to same pid
      false ->
	  {{Free, [{Freq, Pid} | Allocated]},
	   {ok,
	    Freq}}  % first allocation to this Pid, return rest of frequencies
    end.

deallocate({Free, Allocated}, {Freq, Pid}) ->
    case lists:member({Freq, Pid}, Allocated) of
      true ->
	  Allocated2 = lists:delete({Freq, Pid},
				    Allocated),     % this freq did belong to this caller, so release it
	  unlink(Pid),    % client deallocated frequencies, so ignore client if it dies now
	  {{[Freq | Free], Allocated2}, ok};
      false ->
	  {{Free, Allocated},
	   {error,
	    not_allocated}}           % whatcha talkin 'bout bro? u didn't have this freq...
    end.
...
start() ->
    case
      whereis(?MODULE)        % verify if server is already running
	of
      undefined ->
...
stop(Id) ->
    ...
    receive
      {reply, Reply} -> print_client_msg(Id, Reply)
    end.
...
allocate() ->
    ...
    receive
      {'EXIT', _Pid, _Reason} ->
	  io:format("server died", []), {error, atimeout};
      {reply, Reply} ->
	  Reply    % Reply: {ok, Freq} {error,no_frequency} {error,already_allocated}  {error,atimeout}
      after
	?CLIENTTIMEOUT ->        % Don't wait forever, the server may be overloaded
...
deallocate(Freq) ->
    ...
    receive
	  ...
	  Reply
      after
	?CLIENTTIMEOUT ->         % the server must be overloaded
    ...
    receive
      ...
      after 0 ->
		ok           % just like in Erlang Programming p. 106...
                     ...
                     % AFTER ensures it will continue if no msgs are present
...
start_clients(Num_clients) ->      % launch a bunch of clients numbered 1..n, as processes
    ...
    timer:kill_after(15000,
		     whereis(frequency)). % will kill server after 15s.

client_loop(0) -> clients_launched;
client_loop(Num_clients) ->
...
client(Id) ->
    ...
    register(list_to_atom("c" ++ integer_to_list(Id)),
	     self()),    % register client with a name c1, c2...
    ...
    case allocate() of
	  ...
	  timer:sleep(timer:seconds(1) +
			rand:uniform(timer:seconds(4)));
      {ok, Frequency} ->
	  print_client_msg(Id, "A~w~n", [Frequency]),
	  timer:sleep(1000 +
			rand:uniform(4000)),     % hold it for a little random while...
	  ...
	  print_client_msg(Id, "Server died~n");
      Other ->
	  print_client_msg(Id, "Unk result for alloc(): ~p~n",
			   [Other])
    end,
...
print_client_msg(Col, Msg) when is_list(Msg) ->
    ...
    io:format("~s~n",
	      [string:copies(?INDENT, Col - 1) ++ Msg]);
print_client_msg(Col, Msg) when is_atom(Msg) ->
    io:format("~s~n",
	      [string:copies(?INDENT, Col - 1) ++ atom_to_list(Msg)]);
print_client_msg(Col, Msg)
    when is_integer(Msg) -> %frequency
    io:format("~p~n",
	      [string:copies(?INDENT, Col - 1) ++ Msg]);
print_client_msg(Col, Msg) ->
    io:format("~p~n",
...
label(I, N, D) when I < N ->
    io:format("C~w~s",
	      [I, lists:sublist(?INDENT, length(?INDENT) - 2)]),
    label(I + D, N, D).

trap(true) -> process_flag(trap_exit, true);
trap(false) -> process_flag(trap_exit, false).
