...
%% Step 2.10 ASSIGNMENT: Enhancing the frequency server


%% make a simulation to perform a sequence of steps:
    % launch a frequency server from a supervisor process, init it, register it as "frequency":
    ...
    % kill the server and have the supervisor restore it, fix state.
  ...
  % launch clients, which link to frequency server
    case whereis(frequency_monitor) of  % verify if monitor is already running
        undefined ->
            ...
            io:format("~w Launched 'frequency_monitor' (~w)~n",[erlang:unique_integer([monotonic,positive]),MonitorPid]);
...
init_fs() ->
    ...
    io:format("~w FServer up with ~p~n",[erlang:unique_integer([monotonic,positive]),Frequencies]),
    loop(Frequencies).
...
launch_frequency_server()->
    ...
    io:format("~w Launched 'frequency' (~w)~n",[erlang:unique_integer([monotonic,positive]),FServerPid]),
    ok.


monitor_loop(Count) ->
  %io:format("~w Times FS server started: ~w~n",[erlang:unique_integer([monotonic,positive]),Count]),
  receive
    {'EXIT', _Pid, _Reason} ->     % the frequency server has died!
        print_client_msg(1,"Mon: FS died.----------------------------------------------~n",[]),
        launch_frequency_server(),
...
loop(Frequencies) ->                            % The Main Loop
  receive
    ...
    {'EXIT', Pid, _Reason} ->     %% a client linked to the server has exited!
     ...
     {request, Pid, stop} ->
        io:format("~w FS: stop.~n",[erlang:unique_integer([monotonic,positive])]),
...
deallocate(Freq) ->
    ...
    receive
%        {'EXIT', Pid, _Reason} ->
%            print_client_msg(1,"d:Server ~p died.~n",[Pid]),
%            {error,atimeout};
	    {reply, Reply} ->
...
start_clients(Num_clients) ->      % launch a bunch of clients numbered 1..n, as processes
    ...
    _TimerRef=timer:kill_after(15000,whereis(frequency)). % will kill server after 15s, monitor will restart it.
...
print_client_msg(Col,Format,Params) ->
    io:format("~w ~s",[erlang:unique_integer([monotonic,positive]),string:copies(?INDENT,Col-1)]),
...
print_client_msg(Col,Msg) when is_list(Msg)->
    io:format("~w ~s~n",[erlang:unique_integer([monotonic,positive]),string:copies(?INDENT,Col-1)++Msg]);
print_client_msg(Col,Msg) when is_atom(Msg)->
    io:format("~w ~s~n",[erlang:unique_integer([monotonic,positive]),string:copies(?INDENT,Col-1)++atom_to_list(Msg)]);
%print_client_msg(Col,Msg) when is_integer(Msg) -> %frequency
%    io:format("~w ~p~n",[erlang:unique_integer([monotonic,positive]),string:copies(?INDENT,Col-1)++Msg]);
